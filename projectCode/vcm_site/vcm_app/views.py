...
class LeasesView(generic.ListView):
    template_name = 'vcm_app/leases.html'
    context_object_name = 'leases_list'


    """TRYING SOMEWITHING WEIRD HERE
        Passing additional context data to the view"""
    def get_context_data(self, **kwargs):
        context = super(LeasesView, self).get_context_data(**kwargs)
        context.update({
            'worktype_list': WorkType.objects.all(),
        })
        return context
